  ...
  def self.list_recipes(search)
    ...
    if data["hits"]
      my_hits = data["hits"].map do |hit_hash|
        Recipe.new hit_hash["recipe"]["label"], hit_hash["recipe"]["ingredientLines"], hit_hash["recipe"]["image"], hit_hash["recipe"]["url"], hit_hash["recipe"]["source"], hit_hash["recipe"]["ingredients"], hit_hash["recipe"]["uri"]
      end
      return my_hits
    else
      return []
